...
//   const sql = "SELECT * FROM users";
//   db.query(sql, (err, data) => {
//     if (err) return res.status(400).json({ error: "Failed to select", details: err.message });
...
//     if (err) return res.status(500).json({ error: "Failed to insert", details: err.message });
...
//   const { username, email } = req.body;
//   const sql = "UPDATE users SET username = ?, email = ? WHERE id = ?";
//   db.query(sql, [username, email, userId], (err, data) => {
